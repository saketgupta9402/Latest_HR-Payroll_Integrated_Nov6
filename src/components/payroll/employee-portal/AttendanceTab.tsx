  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Loss of Pay Days</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Click on any day to see details. Leaves are shown in colored backgrounds.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center gap-6">
            <Calendar
              ...
              classNames={{
                ...
                day: cn("h-9 w-9 p-0 font-normal aria-selected:opacity-100 rounded-md flex items-center justify-center text-sm cursor-pointer transition-colors hover:bg-accent relative"),
                  ...
                  return (
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          {date.getDate()}
                          {dayData && <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-white" />}
                        </button>
                      </PopoverTrigger>
                      {dayData && (
                        <PopoverContent className="w-80">
                          <div className="space-y-2">
                            <div className="font-semibold">{format(date, "EEEE, MMMM dd, yyyy")}</div>
                            {dayData.type === "leave" && (
                              <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                  <Badge className={getLeaveTypeColor(dayData.leaveType)}>{getLeaveTypeLabel(dayData.leaveType)}</Badge>
                                  <Badge variant="outline">{dayData.data.status}</Badge>
                                </div>
                                <p className="text-sm text-muted-foreground">{dayData.data.reason || "No reason provided"}</p>
                              </div>
                            )}
                            {dayData.type === "attendance" && (
                              <div className="space-y-1">
                                ...
                                {dayData.data.remarks && <p className="text-sm text-muted-foreground">{dayData.data.remarks}</p>}
                                {dayData.data.is_lop && <p className="text-xs text-destructive">This day will result in salary deduction</p>}
                              </div>
                            )}
                          </div>
                        </PopoverContent>
